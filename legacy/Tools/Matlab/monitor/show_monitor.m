  ...
  function update_single( robots, teamNumber, playerNumber )
    ...
    if MONITOR.enable1
      ...
      if MONITOR.is_webots    
        ...
        if yuyv_type==1
          yuyv = robots{playerNumber,teamNumber}.get_yuyv(1);
          plot_yuyv(yuyv); % disp('here3')
          plot_overlay(r_mon,1,1,1);
%             cb = robots{playerNumber,teamNumber}.get_cb(1);
%             disp(size(cb))
%             image(cb)
        elseif yuyv_type==2
...
%          plot_overlay(r_mon,2*MONITOR.enable1,1,2);
        ...
        if yuyv_type==1
          yuyv = robots{playerNumber,teamNumber}.get_yuyv(2);
          plot_yuyv(yuyv); % disp('here4')
          plot_overlay(r_mon,1,1,2);       
%             cb = robots{playerNumber,teamNumber}.get_cb(2);
%             image(cb)
%             disp(size(cb))
        elseif yuyv_type==2
...
% 		end
      ...
      %top camera
		MONITOR.customScatter = subplot(4,7,[5, 6, 12, 13]);
		yuyv = robots{playerNumber,teamNumber}.get_yuyv(1);
		[ycbcr,rgb] = yuyv2rgb(yuyv);
	  
		ycbcr_vec = reshape(ycbcr, [], 3);
		rgb_vec = reshape(rgb, [], 3);
	  
		scatter3(ycbcr_vec(:,1), ycbcr_vec(:,2), ycbcr_vec(:,3), 1, double(rgb_vec)/255);
		view(40,35), xlim([0, 255]), ylim([0, 255]), zlim([0, 255])
		set(subplot(4,7,[5,6,12,13]),'Color',[1,.8,.9])

      %bottom camera
		MONITOR.customScatter2 = subplot(4,7,[19 20 26 27]);
		yuyv = robots{playerNumber,teamNumber}.get_yuyv(2);
		[ycbcr,rgb] = yuyv2rgb(yuyv);
		...
		scatter3(ycbcr_vec(:,1), ycbcr_vec(:,2), ycbcr_vec(:,3), 1, double(rgb_vec)/255);
		view(40,35), xlim([0, 255]), ylim([0, 255]), zlim([0, 255])
		set(subplot(4,7,[19 20 26 27]),'Color',[1,.8,.9])
        xlabel('Y'), ylabel('Cb'), zlabel('Cr')
      
    elseif (MONITOR.enable2==3)
      MONITOR.h2 = subplot(4,7,[5 6 12 13]);
      cb = robots{playerNumber,teamNumber}.get_cb(1);
      imagesc(cb) % disp('here1')
      colormap default
%       colorbar
      if MONITOR.enable0
        plot_overlay(r_mon,r_mon.camera.scaleBtop,MONITOR.enable0,1);
...
%       colorbar
    ...
    elseif (MONITOR.enable2==4)
      %top camera
      MONITOR.h2 = subplot(4,7,[5 6 12 13]);
      cbScaleB = robots{playerNumber,teamNumber}.get_cbScaleB(1);
      imagesc(cbScaleB)
      colormap default
%       colorbar
      if MONITOR.enable0
      ...
      %bottom camera
      MONITOR.h2 = subplot(4,7,[19 20 26 27]);
      cbScaleB = robots{playerNumber,teamNumber}.get_cbScaleB(2);
      imagesc(cbScaleB)
      colormap default
%       colorbar
      if MONITOR.enable0
    ...
    elseif (MONITOR.enable2==5) && (~isempty(MONITOR.lutname))
      MONITOR.h2 = subplot(4,7,[17 18 24 25]);
      yuyv = robots{playerNumber,teamNumber}.get_yuyv(1);
...
%          set(MONITOR.infoTexts(i),'String',infostr);
  ...
  function button2(varargin)
    MONITOR.enable2=mod(MONITOR.enable2+1,5);
    if MONITOR.lutname==0
      MONITOR.enable2=mod(MONITOR.enable2,5);
    end 
    if MONITOR.enable2==1 set(MONITOR.hButton2,'String', 'LABEL A');
    elseif MONITOR.enable2==2 set(MONITOR.hButton2,'String', 'LABEL B');
    elseif MONITOR.enable2==3 set(MONITOR.hButton2,'String', 'CB SCALE A');
    elseif MONITOR.enable2==4 set(MONITOR.hButton2,'String', 'CB SCALE B');
    elseif MONITOR.enable2==5 set(MONITOR.hButton2,'String', 'LUT');
