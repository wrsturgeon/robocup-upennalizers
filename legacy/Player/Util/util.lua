...
-- returns an equivalent angle with an absolute value less than or equal to pi
...
function pose_global(pRelative, pose)
  ...
  return vector.new{pose[1] + ca*pRelative[1] - sa*pRelative[2], --pose x + projection of pRelative onto global x axis
                    pose[2] + sa*pRelative[1] + ca*pRelative[2], --pose y + projection of pRelative onto global y axis
                    pose[3] + pRelative[3]};
end

-- gets x and y components of vector from pose to pGlobal and angle from pose to pGlobal
-- from reference frame of pose
