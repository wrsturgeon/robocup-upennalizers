...
if usewebots then
  naodynamics.initialize("Player/Motion/Xiang_Dynamicslib/urdf/naofullbody.urdf","Player/Motion/Xiang_Dynamicslib/keyframes/standupFromFront.motion")
...
function heeltoecomp(supportLeg)
  local kp=10;
  comp={0,0}; --ap ar
  if  supportLeg==0 then
    local err=(ft_right_f[3]-ft_right_b[3]);
    comp[1]=err*kp;
  else
    local err=(ft_left_f[3]-ft_left_f[3]);
    comp[1]=err*kp;
  end
  return comp;
...
function getupfromFront()
  ...
  if getup_active then
    ...
    if not getup_started then
      ...
      qcontrol=naodynamics.standupFromFrontUpdate(tcur*1000,holdtime,cfrid)
      ...
      qcontrol[11+6]=qcontrol[11+6]+comp[1];
      ...
      if dogetup==0 then
        if false then
          compute_diffm();
          if ( (cp.bodyHeight- diff_m[3])>0.05) then
            print('cp.bodyHeight, diff_m[3]',cp.bodyHeight,diff_m[3])
            dogetup=1;

            -- Final stance foot position6D
            pTorsoTarget = vector.new({Config.walk.supportX, 0, cp.bodyHeight,
                0,0,0});
            pLLeg = vector.new({-cp.supportX , cp.footY, 0, 0,0,0});
            pRLeg = vector.new({-cp.supportX , -cp.footY, 0, 0,0,0});
            ...
            pTorso=pTorso+dpTorso*deltaT*10;
            pTorsoActual = {
              pTorso[1],
              pTorso[2],
              pTorso[3],
              ...
              pTorso[5],
              pTorso[6]}
            qLeg = Kinematics.inverse_legs(pLLeg, pRLeg, pTorsoActual, 0);
            ...
            qcontrol[3]=math.pi/2;
            qcontrol[19]=math.pi/2;
            for i=1,#qLeg do
              qcontrol[6+i]=qLeg[i];
            end
          ...
          qLArm_to[1]= math.pi/2+0.45;
          qRArm_to[1]= math.pi/2+0.45;
          qLArm_to[2]=0.3;
          qRArm_to[2]=-0.3;
          -- Body.set_larm_command(qLArm_to);
          -- Body.set_rarm_command(qRArm_to);
          -- Body.set_actuator_command(19, math.pi/2-0.2);

          Body.set_head_command({0,-0.5})
          -- Body.set_head_hardness(0.6)
          -- qall[3]=math.pi/2-0.2;
          -- qall[19]=math.pi/2-0.2;
          -- Body.set_actuator_command(qall);
          Body.set_lleg_hardness(0.5);
          Body.set_rleg_hardness(0.5);
          Body.set_larm_hardness(0.3);
          Body.set_rarm_hardness(0.3);
      ...
      getup_active=dogetup>0 and not checkheight;
      getup_started=dogetup>0 and not checkheight;
      -- print('qcontrol',unpack(qcontrol))
      if getup_started then
        ...
        motion_legs({unpack(qcontrol,7,18)})
        Body.set_head_command({unpack(qcontrol,1,2)})
        -- Body.set_actuator_command(qcontrol)
      end
    end
  end
...
function normalloop()
  ...
  uTorsoActual = util.pose_global(vector.new({0,0,0}),uTorso)

  ---------------XIANG's TODO--------------------------------------
  -- desired com wrt support foot
  compute_diffm();
  local com_d;
  if supportLeg==0 then
    com_d=util.pose_relative(uTorso,uLeft);
  else
    com_d=util.pose_relative(uTorso,uRight);
  end
  local uToso_comp={0,0,0}
  print('com_d',unpack(com_d))
  if math.abs(imuAngle[2])<0.3 then
    uToso_comp[1]=0.1*(com_d[1]-diff_m[1])* math.sqrt(1-ph*ph);
    uToso_comp[2]=0.1*(com_d[2]-diff_m[2]) ;
  end
  print('uToso_comp',unpack(uToso_comp))
  print('deltaT',deltaT)

  -- supportXauto= math.min(supportXauto+ 0.005*(0.02-diff_m[1]),0.03);
  -- print('supportXauto',supportXauto)
  uTorsoActual = util.pose_global(uToso_comp,uTorso)
  ...
  if math.abs(imuAngle[2])<0.7 and false then
    pTorso = vector.new({uTorsoActual[1], uTorsoActual[2], cp.bodyHeight,1*fact*uToso_comp[2]+0,2*fact*uToso_comp[1]+0,uTorsoActual[3]});
  else
    pTorso = vector.new({uTorsoActual[1], uTorsoActual[2], cp.bodyHeight, 0, 0,uTorsoActual[3]});
  end
  --------------------------------------------------------
  ...
  if walkKickRequest>0 then
    ...
    elseif kick_stage ==2 then
      ...
      if true then
        ...
        if true then
          local fact=0;
          local val =math.min(0.03,math.sqrt(math.abs(vest_roll)/0.3)*0.01);
        ...
        if true then
          local fact=0;
          local val =math.min(0.03,math.sqrt(math.abs(vest_pitch)/0.3)*0.01);
          -- local val =math.min(0.05,math.sqrt(math.abs(imuAngle[2])));
