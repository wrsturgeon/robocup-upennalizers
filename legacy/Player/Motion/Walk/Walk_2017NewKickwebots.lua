...
is_stopped = false
stop_threshold = {10*math.pi/180,20*math.pi/180}
tStopStart = 0
tStopDuration = 2.0
supportLeg=0;
ike = false;
...
function normalloop()
  ...
  local locE=(math.abs(imuAngle[2]-0.05)*0.3*8+0.5*5*vest_pitch*vest_pitch);
  -- TODO replace this by COM deviation
  -- print(vest_pitch,locE)
  if cnter % 50 ==0 then
    --print(vest_pitch,locE)
  end
  unstable_factor=locE/stop_E;
  max_unstable_factor = math.max(unstable_factor, max_unstable_factor)

  --start emergency stop
  if unstable_factor>1 and walkKickRequest==0 then
    tStopDuration= math.sqrt(locE/4)
    print(tStopDuration)
    stopRequest = 2
    tStopStart = t
    velCurrent= {0,0,0}
  ...
  -- print('uLeft',uLeft,'uLeft1',uLeft1,'uLeft15',uLeft15,'uLeft2',uLeft2)
  -- print('uRight',uRight,'uRight1',uRight1,'uRight15',uRight15,'uRight2',uRight2)
  -- end
  ...
  pTorso = vector.new({uTorsoActual[1], uTorsoActual[2], cp.bodyHeight, 0,pitch2,uTorsoActual[3]});

  -- if kick_strike and supportLeg==0 then
  -- pTorso[5]=pTorso[5]+0.15;
  -- end
...
function plan_nextstep()
  if (iStep > iStep0) then
    ...
    -- if supportLeg==0 then
    -- print(imuAngle[2],ZMPFl)
    -- else
    -- print(imuAngle[2],ZMPFr)
    -- end
    --Switch walk params
    cp = np
    np = load_default_param_values()
    -- np.stepHeight=0.016;---- *<<<<<<PARAM>>>>>/
    if walkKickRequest==0 and kick_stage==0 then
...
function xiang_walkkick_0step()
  ...
  if walkKickRequest>0 then
    ...
    elseif kick_stage ==2 then
      ...
      footPos1={0.00,0.02,0.0};
      footPos2={0.00,-0.02,0.0};
      if supportLeg == 0 then
...
function check_walkkick()
  ...
  print('walkKick[walkKickRequest]',unpack(walkKick[walkKickRequest]))
  print('(walkKick[walkKickRequest][5])',unpack(walkKick[walkKickRequest][5]))

  print('#walkKick[walkKickRequest]',#walkKick[walkKickRequest])
  if #walkKick[walkKickRequest] <=7 then
    ...
    print('B')
    print('footPos1',unpack(footPos1),'footPos2',unpack(footPos2))
    if supportLeg == 0 then
      uRight15 = util.pose_global({footPos1[1],footPos1[2]-2*cp.footY,footPos1[3]},uLeft1)
      uRight2 = util.pose_global({footPos2[1],footPos2[2]-2*cp.footY,footPos2[3]},uLeft1)
    else
...
function motion_legs(qLegs,gyro_off)
  ...
  local imuAngle=Body.get_sensor_imuAngle();
  if (imuAngle[2]<-0.1) then
    if (supportLeg==0) then
      qLegs[3] = qLegs[3] + (1-ph)*imuAngle[2];
    else
      qLegs[3+6] = qLegs[3+6] + (1-ph)*imuAngle[2];
    end
  end
...
function update_velocity()
  ...
  -- if gcm.get_team_role() == [role that you want to debug] then
  --   print("velCurrent: ", velCurrent[1], velCurrent[2], velCurrent[3])
  -- end
end
