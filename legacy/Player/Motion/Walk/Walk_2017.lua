...
is_stopped = false
stop_threshold = {10*math.pi/180,30*math.pi/180}
tStopStart = 0
tStopDuration = 2.0
...
uLeftoff = vector.new({0, Config.walk.footY, 0});
uRightoff = vector.new({0, -Config.walk.footY, 0});
-------------------------------------------------------
...
function update()
  ...
  if (iStep > iStep0) then
    ...
    -- print('velCurrent[1]',velCurrent[1],'\n')
    local vf=velCurrent[1];
    if vf>0 then
      if velCurrent[3]==0 then
        velCurrent[3]= walk_dir_compens;
      end
    elseif vf <0 then
      velCurrent[1]=math.max(vel_backwds,velCurrent[1]);
    else
    end
    ...
    check_walkkick();
    if  walkKickRequest>0 then
      if not  kickrequested  then
        t_kickstart=Body.get_time();
        kickrequested=true;
      end
    end
    if walkKickRequest==0 then
      if kickrequested then
        kickrequested=false;
      end
    end
    -- if isrotating and math.abs(velCurrent[3])<=walk_dir_compens then --------------------Xiang debug
    --   uTorso = vector.new({Config.walk.supportX, 0, 0});
    --   uLeftoff=uLeftoff+uLeft;
    --   uRightoff=uRightoff+uRight;
    --   uLeft = vector.new({0, Config.walk.footY, 0});
    --   uRight = vector.new({0, -Config.walk.footY, 0});
    --   velCurrent, velCommand,velDiff = vector.new({0,0,0}),vector.new({0,0,0}),vector.new({0,0,0});
  ...
  if math.abs(velCurrent[3])>0.1 then
  ...
  local kk= 1;
  local bb=0.1;
  local u_pitch=kk*(imuAngle[2] )- bb* vest_pitch;
  local u_roll=kk*(imuAngle[1] )- bb* vest_roll;
  -- ik_pTorso[1]=ik_uTorso[1]+ 0.04*math.tanh(1*u_pitch);
  -- ik_pTorso[5]=ik_pTorso[5]+ 0.1*math.tanh(2*u_pitch) ;
  -- ik_pTorso[5]= 0.1*math.tanh(- u_pitch) ;
  local adj_ftx= 0.04*math.tanh( u_pitch)-0.02;
  local adj_ftz= 0.02*math.tanh(  math.abs(u_pitch))+0.02;
  local adj_fty= 0.04*math.tanh( -u_roll) ;
  local adj_ftz2= 0.02*math.tanh(  math.abs(u_roll))+0.02;

  local forwardonly=velCurrent[1]>0 and velCurrent[2]==0 and math.abs(velCurrent[3])<0.1;
  local backwardonly=velCurrent[1]<0 and velCurrent[2]==0 and math.abs(velCurrent[3])<0.1;
  local hasforward=velCurrent[1]>0  ;
  local hasbackward=velCurrent[1]<0    ;
  -- local nofb=velCurrent[1]==vel_zero_fwoff;
  local hasleft=  velCurrent[2]>0 and velCurrent[3]==0;
  local hasright= velCurrent[2]<0 and velCurrent[3]==0;
  local kicknow=(xFoot/walkKickPh)<1 and current_step_type>1;
  -- print('forwardonly',forwardonly,'\n')
  -- print('hasforward',hasforward,'\n')
  -- print('hasbackward',hasbackward,'\n')
  local mmm=0.04;
  -- print('here',(t_kickstart),'\n')
  local tmaxpost=1;
  -- local post_kick=walkKickRequest==0 and (curt-t_kickstart)<tmaxpost ;
  local post_kick=(xFoot/walkKickPh)>1 and current_step_type>1;
  if   post_kick then
    stop_threshold = {15*math.pi/180,50*math.pi/180};
    velCurrent[1]=math.tanh(math.abs(2*imuAngle[2]))*0.2;
    velCurrent[2]=0;
    velCurrent[3]=0;
    mmm=0.3;
  -- print('velCurrent[1]',velCurrent[1],'supportLeg',supportLeg,'\n')
  else
    stop_threshold = {15*math.pi/180,35*math.pi/180};
  end
  pLLeg = vector.new({uLeft[1], uLeft[2], zLeft, 0,0,uLeft[3]});
  pRLeg = vector.new({uRight[1], uRight[2], zRight, 0,0,uRight[3]})

  if math.abs(imuAngle[2])>0.05  and not (isrotating) then

    -- print('velCurrent[1]',velCurrent[1],'\n')
    --     print('walkKickRequest',walkKickRequest,'\n')
    if supportLeg==0 and math.abs(velCurrent[1]) < 0.2 or post_kick then ---LS supprt
      if post_kick then
      -- print('print(post_kick)',post_kick,'\n')
      end
      pLLeg[1]=pLLeg[1]+mmm*adj_ftx;
      pLLeg[2]=pLLeg[2]+mmm*adj_fty;
      pLLeg[3]=pLLeg[3]+mmm*adj_ftz;
    elseif supportLeg==1 and math.abs(velCurrent[1]) < 0.2 or post_kick then
      if post_kick then
      -- print('print(post_kick)',post_kick,'\n')
      end
      pRLeg[1]=pRLeg[1]+mmm*adj_ftx;
      pRLeg[2]=pRLeg[2]+mmm*adj_fty;
      pRLeg[3]=pRLeg[3]+mmm*adj_ftz;
    end
  end

  pTorso = vector.new({uTorsoActual[1], uTorsoActual[2], cp.bodyHeight, 0,cp.bodyTilt,uTorsoActual[3]});
  -- pTorso[1]=pTorso[1]-0.01
  ...
  pTorso[3]=pTorso[3]+0.0015*(1-math.cos(phSingle*2*math.pi));---<<<Param>>>>>>Xiang
  if hasforward then
    pTorso[5]=pTorso[5]+0.02;
  end
  if hasbackward then
    pTorso[5]=pTorso[5]+0.05;
  end

  qLegs = Kinematics.inverse_legs(pLLeg, pRLeg, pTorso, supportLeg);
  local toe_factor=0;
  if hasforward or current_step_type>1 then
    toe_factor=1;
    if hasleft or hasright then
      toe_factor=toe_factor/1.4;
    end
  elseif hasbackward then
    toe_factor= 1;
    if hasleft or hasright then
      toe_factor=toe_factor/2;
    end
  end
  if isrotating then
    toe_factor=0;
  ...
  local dq_lift= 0.1*math.sin(phSingle*math.pi);
  if not active then --Double support
  elseif supportLeg == 0 then  -- Left support
    qLegs[11]=qLegs[11]-toe_factor*dq_lift;
    if forwardonly and usetoesupport then
      qLegs[5]=qLegs[5]+0.6*toe_factor*dq_lift;
    end
  else
    qLegs[5] = qLegs[5]  -toe_factor*dq_lift;
    if forwardonly and usetoesupport then
      qLegs[11]=qLegs[11]+0.6*toe_factor*dq_lift;
    end
  end
  ...
  if kicknow then
    ...
    for i=2,6 do
      if supportLeg==0 then
        qLegs[i+6]=qLegs[i+6]-bbb*qds_r[i];
      else
        if i==2 then
          -- print('qds_l', i,(qds_l[i]),qLegs[i],bbb*qds_l[i],'\n')
        end
        qLegs[i]=qLegs[i]-bbb*qds_l[i];
      end
    end
  end
  ...
  if usearm then
    ...
    for i=1,1 do
      if supportLeg==0 then
        qLArm_to[i] = multiplier*-0.02*math.cos(phSingle*math.pi-math.pi/2);
        ...
        qRArm_to[i] = multiplier*-0.02*math.cos(phSingle*math.pi-math.pi/2) ;
      end
    end
