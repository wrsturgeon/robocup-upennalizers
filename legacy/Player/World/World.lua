...
function init_particles_manual_placement()
  ...
  --regular players get initialized on the field
    ...
    print('gcm.get_game_kickoff() ',gcm.get_game_kickoff() )
...
function update_pos()
  ...
  log.debug("Odom Testing - no vision updates")
...
--For using gps stuff
  ...
  if use_team_ball>0 and
    ...
    -- print("TEAMBALL")
    wcm.set_robot_use_team_ball(1);
  end --end use_team_ball

    update_shm();
    return;
  end --end use_gps_only
...
--Actual non-gps stuff
  ...
  -- Penalized?
--[[
  if gcm.in_penalty() and gcm.get_game_controllerState() ~= 2 then
...
--Penalty with Motion in set dealt with
  if gcm.in_penalty() and gcm.get_game_controllerState() ~= 2 then
    wcm.set_robot_penalty(1);
    update_shm();
    if wcm.get_robot_resetOrientation() == 1 then
  ...
  elseif gcm.in_penalty() and gcm.get_game_controllerState() == 2 then
      if wcm.get_robot_penalty() == 1 then
         ...
         log.debug("Motion in Set");
         wcm.set_robot_penalty(-1);  --motion in set penalty
         update_shm();
         return
      end
  else
      if wcm.get_robot_penalty() == 1 and gcm.get_game_controllerState() ~= 2 then
         init_particles()
	     wcm.set_robot_penalty(0);
      elseif wcm.get_robot_penalty() == -1 and gcm.get_game_controllerState == 3 then
         wcm.set_robot_penalty(0);
         update_shm();
      end
  end
  ...
  -- print('gcm.get_game_state()',gcm.get_game_state())
  if gcm.get_game_state()==0 then
    ...
    for i=1,#fsrRight do
      fsrRights[i]=fsrRights[i]+fsrRight[i]
      fsrLefts[i]=fsrLefts[i]+fsrLeft[i]
    end
    cntf=cntf+1;
  elseif gcm.get_game_state==1 and calibratenotdone   then
    calibratenotdone=false;
    for i=1,#fsrRight do
      fsrRight0[i]=fsrRights[i]/cntf;
      fsrLeft0[i]=fsrLefts[i]/cntf;
    end
  end
  fsrRight = Body.get_sensor_fsrRight()
  fsrLeft = Body.get_sensor_fsrLeft()
  ...
  -- print('calibratenotdone',calibratenotdone)
  -- print('fsrRights',vector.new(fsrRights))
  -- print('fsrRight0',vector.new(fsrRight0))
  ...
  if (not webots or webots==0) and gcm.get_team_role()~=5 and gcm.get_game_state() ==2 then
    ...
    local r20=0;
    for i=1,#fsrRight do
      -- fsrRight[i]=fsrRight[i]-fsrRight0[i]
      -- fsrLeft[i]=fsrLeft[i]-fsrLeft0[i]
      r1=r1+fsrLeft[i]
      r2=r2+fsrRight[i];
      r10=r10+fsrLeft0[i]
      r20=r20+fsrRight0[i]
    end
    local liftedcond=(r1-r10)/r10<0.2 and (r2-r20)/r20<0.2;
    -- print('r10',r10,'r1',r1,'r20',r20,'r2',r2)

    curT = Body.get_time();

    --check if we are in the air
    if allLessThanTenth(fsrRight) and allLessThanTenth(fsrLeft) or liftedcond then --DX
...
--    print('VCM:', v[1], v[2], dr, da)
  ...
  if vcm.get_line2_detect() == 1 and vcm.get_circle_detect() == 0 and vcm.get_corner2_detect() == 0 and gcm.get_team_role()~=5 then
 ...
 if vcm.get_line1_detect() == 1 and vcm.get_circle_detect() == 0 and vcm.get_corner1_detect() == 0 and gcm.get_team_role()~=5 then
  ...
  --circle detection
  if vcm.get_circle1_detect() == 1 and gcm.get_team_role()~=5 then
    ...
    --print('Circle Update');
    --print("x:" .. x, "y:" .. y);
    PoseFilter.topCircleUpdate(v,a);
  end

  if vcm.get_circle2_detect() == 1 and gcm.get_team_role()~=5 then
    local  x = vcm.get_circle_y(); --x and y are flipped because vision reports them backwards
    local  y = vcm.get_circle_x();
    local  v = {};
    local  a = vcm.get_circle_angle();
    ...
    --print('Circle Update');
    --print("x:" .. x, "y:" .. y);
    PoseFilter.btmCircleUpdate(v,a);
  end

  --Bottom corner detection
  if vcm.get_corner2_detect() == 1 and gcm.get_team_role()~=5 then
    --L corner
    if vcm.get_corner2_type() == 1 then
        local v=vcm.get_corner2_v();
        local a=vcm.get_corner2_angle();
        PoseFilter.cornerLBtm(v,a);
    --T corner
    elseif vcm.get_corner2_type() == 2 then
        local v=vcm.get_corner2_v();
        local a=vcm.get_corner2_angle();
        PoseFilter.cornerTBtm(v,a);
    end
  end

   --Top corner detection
  if vcm.get_corner1_detect() == 1 and gcm.get_team_role()~=5 then
    --L corner
    if vcm.get_corner1_type() == 1 then
        local v=vcm.get_corner1_v();
        local a=vcm.get_corner1_angle();
        PoseFilter.cornerLTop(v,a);
    --T corner
    elseif vcm.get_corner1_type() == 2 then
        local v=vcm.get_corner1_v();
        local a=vcm.get_corner1_angle();
        PoseFilter.cornerTTop(v,a);
    end
  end

  --Bottom spot detection
  if vcm.get_spot2_detect() == 1 and gcm.get_team_role()~=5 then
    local color = vcm.get_spot2_color();
    local v = vcm.get_spot2_v();
    if color == Config.color.white then
      PoseFilter.bottomSpotUpdate(v);
    end
  end

  --Top spot detection
  if vcm.get_spot1_detect() == 1 and gcm.get_team_role()~=5 then
    local color = vcm.get_spot1_color();
    local v = vcm.get_spot1_v();
    if color == Config.color.white then
      PoseFilter.topSpotUpdate(v);
    end
  end
