...
ballUsed = -1; --same usage as ballUsed in headScan
...
--  print('Updating headtrack')
  ...
  ball = wcm.get_ball();
	...
	-- set eye LED to denote whether the ball is local/team ball
  if wcm.get_robot_use_team_ball() == 0 then
		if ballUsed ~= 0 then
      print('headScan: using local ball')
      ballUsed = 0;
    end

	--if we are using team ball we can look towards it if we are moving
  elseif wcm.get_robot_use_team_ball() == 1 --and mcm.get_walk_isMoving() == 1
 		then
		if ballUsed ~= 1 then
      print('headTrack: using team ball')
      ballUsed = 1;
    end
  end
  ...
  --if ((Body.get_head_position()[1] < yaw + yawErr and Body.get_head_position()[1] > yaw - yawErr) and
	...
	if	(t - ball.t > tLost) or (ballUsed == 1 and (role == 2 or role == 4) and vcm.get_ball_detect()==0) then
   	print('Ball lost!');
