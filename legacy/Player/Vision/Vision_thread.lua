...
--local CameraSettingsTool = require('CameraSettingsTool');
...
local camera_setting_Naov4 = function(self, cidx)
  for i,param in ipairs(Config.camera.param) do
    print('Camera '..cidx..': setting '..param.key..': '..param.val[cidx]);
    self.camera:set_param(param.key, param.val[cidx], cidx-1);
    unix.usleep (100);
    print('Camera '..cidx..': set to '..param.key..': '..
            self.camera:get_param(param.key, cidx-1));
  end

--  local auto_exposure = self.camera:get_param('Auto Exposure', cidx-1);
--  if auto_exposure == 1 then
--    self.camera:set_param('Auto Exposure Algorithm', Config.camera.auto_exposure_algorithm, cidx-1);
--    self.camera:set_param('Brightness', Config.camera.brightness, cidx-1);
--  end

  print('Camera #'..cidx..' set');
end
...
local update = function(self)
  ...
  --DP------------------------------------
  --local y  = ImageProc.yuyv_to_ycbcr(self.image, 1)
  --local cb = ImageProc.yuyv_to_ycbcr(self.image, 2)
  --local cr = ImageProc.yuyv_to_ycbcr(self.image, 3)
  --local y_scaleB  = ImageProc.yuyv_to_ycbcr_scaleB(self.image, 1)
  --local cb_scaleB = ImageProc.yuyv_to_ycbcr_scaleB(self.image, 2)
  --local cr_scaleB = ImageProc.yuyv_to_ycbcr_scaleB(self.image, 3)

  --local labelA_t = torch.zeros(y:size()):byte()

  --local y_ptr  = cutil.torch_to_userdata(y)
  --local cb_ptr = cutil.torch_to_userdata(cb)
  --local cr_ptr = cutil.torch_to_userdata(cr)
  --local y_scaleB_ptr  = cutil.torch_to_userdata(y_scaleB)
  --local cb_scaleB_ptr = cutil.torch_to_userdata(cb_scaleB)
  --local cr_scaleB_ptr = cutil.torch_to_userdata(cr_scaleB)

  --local labelA_ptr = cutil.torch_to_userdata(labelA_t)

  --local im_size = y:size(1) * y:size(2)
  --local im_size_scaleB = y_scaleB:size(1) * y_scaleB:size(2)

  --local nn_prediction = ImageProc.nn_predict(labelA_ptr, y_ptr, cb_ptr, cr_ptr, im_size,
  --                                    y_scaleB_ptr, cb_scaleB_ptr, cr_scaleB_ptr, im_size_scaleB)
  --print('printing nn output in lua')
  --for k,v in ipairs(nn_prediction) do
  --  print(v)
  --end
  ...
  --self.CameraSettingsTool:update(self); -- Do 2019 get/set autoexposure config settings
...
function Vision.entry(cidx)
  ...
  -- Initiate CustomAutoExposure (CameraSettingsTool) 
  --self.CameraSettingsTool = CameraSettingsTool.entry(self)
