...
rFar = 0.60 --this was set to 0.45, which is too short
...
function check_approach_type()
  ...
  if do_evade_kick then
    ...
    if (pose.y > 0 and goalDefend[1] > 0) or (pose.y < 0 and goalDefend[1] < 0) then
      kick_type = 2;
  ...
  --   position.posCalc();
  --   pose = wcm.get_pose();
  --   aGoal = wcm.get_goal_attack_angle2(); --Global angle to the goal
  ...
  elseif kick_type == 2 then --walkkick
    if kick_dir == 1 then --Front kick
      ...
      --if sign(ball.y) < 0 then y_inv = 1; end -- for now only do left walkkick; none of the robots do right walkkick well
    elseif kick_dir == 2 then --Kick to the left
...
function entry()
  ...
  print("kick type and dir: ", kick_type, kick_dir)
  print("xTarget: ", xTarget[2])
  print("yTarget: ", yTarget[2])
...
function update()
  ...
  if Config.fsm.playMode == 1 or is_confused > 0 then
    ...
    ballA = math.atan2(
      ball.y - math.max(math.min(ball.y, 0.05), -0.05),
      ball.x + 0.10);
    vStep[3] = 0.5*ballA;
    targetangle = 0;
    angleErrL = 0;
  ...
  if math.abs(wAngle) > 45*math.pi/180 then
    ...
    --   -- print("backstep right")
    --   vStep[2] = 0.03;
    -- else
    --   -- print("no y vel")
    --   vStep[2] = 0;
    -- end
    -- print("instead of doing dumb shit i'm transitioning to bodyOrbit")
    return "ballOrbit"
  else
  ...
  -- print("approach v", vStep[1], vStep[2], vStep[3])
  walk.set_velocity(vStep[1], vStep[2], vStep[3]);
  -- if Config.fsm.velocity_testing then
  --    log.warn(vStep[1], " ", vStep[2], " ", vStep[3], " 2");
  -- end

  if (t - ball.t > tLost) and role>0 then
    HeadFSM.sm:set_state('headScan');
    log.info("transition: ballLost");
    return "ballLost";
  end
  ...
  if (t - t0 > timeout) then
    ...
    log.info("transition: timeout");
    return "timeout";
  end
  if (ballR > rFar) then
    HeadFSM.sm:set_state('headTrack');
    log.info("transition: ballFar", ballR, rFar);
    print("ballR > rFar; transitioning out of bodyApproach")
    return "ballFar";
  end

  angle_check_done = true;
  if check_angle > 0 and (angleErrL > 0 or angleErrR > 0) then
  ...
  if kick_dir == 1 then --Front kick
    ...
    if (ball.x < xTarget[3]) and (t-ball.t < 0.5) and
      ...
      if kick_type == 1 then
        log.info("transition: kick");
        return "kick";
      else
        log.info("transition: walkkick");
        return "walkkick";
      end
    end
  else
