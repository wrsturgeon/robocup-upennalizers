...
function get_sensor_position(index)
	if (index) then
		...
		return controller.wb_motor_get_target_position(controller.wb_robot_get_device(jointNames[index]));
		...
		for i = 1,nJoint do
			...
			t[i] = controller.wb_motor_get_target_position(controller.wb_robot_get_device(jointNames[i]));
		end
		return t;
	end
end
...
function update()
	...
	for i = 1,nJoint do
		...
		if actuator.hardness[i] > 0 then
			...
			controller.wb_motor_set_position(tags.joints[i],
			actuator.position[i]);
