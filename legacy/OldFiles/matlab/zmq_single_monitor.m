...
cams{1}.h_debug = annotation('textbox',...
    [0.5 0.45 0.5 0.55],...
    'FontSize',14,...
    'FontName','Arial',...
    'LineStyle','--',...
    ...
    'Color',[0.84 0.16 0]);
% Camera 2 debug
cams{2}.h_debug = annotation('textbox',...
    [0.5 0 0.5 0.45],...
    'FontSize',14,...
    'FontName','Arial',...
    'LineStyle','--',...
...
DB_SELECT = 0;
h_button = uicontrol('Style','pushbutton','String','Goal',...
   'Units','Normalized', 'Position',[0.9 0.95 0.1 0.05],'Callback',@debug_select);
    ...
    for s=1:numel(idx)
        ...
        if isfield(cam, 'fd')
            while udp_recv('getQueueSize',cam.fd) > 0
                ...
                process_monitor_message(metadata, raw, cam, s_idx_m);
            end
        else
            while 1
                % Receive
                ...
                % Process the raw data
                raw = data(offset+1:end); % This must be uint8
                [has_draw] = process_monitor_message(metadata, raw, cam, s_idx_m);
