...
itemReject = 'yuyv, yuyv2, labelA, labelB, map, cb, cbScaleB'
...
function sendCb_scaleB()
  for nc = 1, Config.camera.ncamera do
    local cb = vcm['get_image'..nc..'_cbScaleB']()
    local width = vcm['get_image'..nc..'_width']() / Config.vision.scaleB[nc]
    local height = vcm['get_image'..nc..'_height']() / Config.vision.scaleB[nc]
    local count = vcm['get_image'..nc..'_count']()
    local array = serialization.serialize_array(cb, width, height, 
    'uint8', 'cbScaleB'..tostring(nc), count);

    local sendcb = {};
    sendcb.team = {};
    sendcb.team.number = gcm.get_team_number();
    sendcb.team.player_id = gcm.get_team_player_id();
    ...
    for i=1,#array do
      sendcb.arr = array[i];
      t0 = unix.time();
      senddata=serialization.serialize(sendcb);     
      senddata = Z.compress(senddata, #senddata);
      t1 = unix.time();
    ...
    if debug>0 then
      print("cb info array num:",#array,"Total size",totalSize);
...
function sendCb()
  for nc = 1, Config.camera.ncamera do
    local cb = vcm['get_image'..nc..'_cb']()
    local width = vcm['get_image'..nc..'_width']() / Config.vision.scaleA[nc]
    local height = vcm['get_image'..nc..'_height']() / Config.vision.scaleA[nc]
    local count = vcm['get_image'..nc..'_count']()
    local array = serialization.serialize_array(cb, width, height, 
    'uint8', 'cb'..tostring(nc), count);
