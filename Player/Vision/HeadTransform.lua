...
function entry(sel)
  ...
  nyA = labelA.n;
  y0A = 0.5 * (nyA - 1);
  if Config.ht_calib_vals then
    if sel == 1 then
      focalA = Config.ht_calib_vals[4]
    elseif sel == 2 then
      focalA = Config.ht_calib_vals[5]
    end
  else
...
--[[ 	Axes: {perpendicular to chest button, perpendicular to side, perpendicular to floor}
  ...
  if Config.ht_calib_vals then
    bodyHeight = Config.ht_calib_vals[1];
    bodyTilt = Config.ht_calib_vals[2];
    neckZ = Config.ht_calib_vals[3];
  else
  ...
  if Config.dev.body == 'SimBody' then
       tNeck = Transform.trans(-footX,0,bodyHeight); -- (0,0,.32) , 0.32 is CoM
