...
local update = function(self, color, p_vision, line)
  ...
  if line.nLines>2 and angle_variance>5 then
	...
	-- checking top camera line variance to get rid of false positive corners on bottom camera
	if p_vision.camera_index == 2 and vcm.get_line2_detect() == 1 and vcm.get_line1_detect() == 1 and vcm.get_line1_nLines()>99 then
		angle_set = {}
  	angle_sum = 0;
		local nLines_top = vcm.get_line1_nLines()
		local angleTable_top = vcm.get_line1_angleTable()

  	for i=1,nLines_top do
    	angle_set[i] = math.abs(180*(angleTable_top[i]%(math.pi/2))/math.pi);
    	if angle_set[i] > 45 then
      	angle_set[i] = 90-angle_set[i];
    	end
    	angle_sum = angle_sum+angle_set[i];
  	end
  	angle_avg = angle_sum/nLines_top;
  	angle_sum_sq = 0
  	angle_variance = 0
  	for i=1,nLines_top do
    	angle_sum_sq = angle_sum_sq + (angle_set[i]-angle_avg)^2
  	end
  	angle_variance = math.sqrt(angle_sum_sq/nLines_top);
  	if nLines_top>3 and angle_variance>5 then
    	--p_vision:add_debug_message(string.format("no bottom corner b/c top variance too large: %.2f.\n",angle_variance));
    	return
  	end
	end
  ...
  for i=1,line.nLines-1 do
    for j=i+1,line.nLines do
      ...
      if math.abs(ang-math.pi/2)<10*math.pi/180 then
        --p_vision:add_debug_message(string.format("Corner Angle: %f \n", math.abs(ang-math.pi/2) * 180/math.pi));
        ip = line.endpoint[i];
        jp = line.endpoint[j];
				--print(ip[1],ip[3],ip[2],ip[4],jp[1],jp[3],jp[2],jp[4])
...
local update_shm = function(self, parent_vision)
  ...
  vcm['set_corner'..cidx..'_detect'](self.detect);
  if (self.detect == 1) then
    vcm['set_corner'..cidx..'_type'](self.type);
    vcm['set_corner'..cidx..'_vc0'](self.vc0);
    vcm['set_corner'..cidx..'_v10'](self.v10);
    vcm['set_corner'..cidx..'_v20'](self.v20);
    vcm['set_corner'..cidx..'_v'](self.v);
    vcm['set_corner'..cidx..'_v1'](self.v1);
    vcm['set_corner'..cidx..'_v2'](self.v2);
    vcm['set_corner'..cidx..'_angle'](self.angle);
  end
end
