...
max_width=Config.vision.line.max_width;
connect_th=Config.vision.line.connect_th;
max_gap=Config.vision.line.max_gap or 1;
min_length=Config.vision.line.min_length;

local update = function(self, color, p_vision)
  self.detect = 0;
  
  cidx = p_vision.camera_index;
  
  linePropsB = ImageProc.field_lines(p_vision.labelB.data, p_vision.labelB.m,
  p_vision.labelB.n, max_width[cidx], connect_th[cidx], max_gap, min_length[cidx]);
  ...
  for i=1,nLines do
    ...
    -- Check if the line is actually a Goalpost (or a robot arm)
    -- if it is a vertical line and goes all the way to top (in practice every line with delta x of 3 is excluded, which applies to most vertical lines)
    -- effectively this check disables most vertical lines since robot arm would always show as lines otherwise

    if valid then
      -- p_vision:add_debug_message(string.format("L%d:(%d %d)(%d %d) ", i, self.propsB[i].endpoint[1],self.propsB[i].endpoint[3],self.propsB[i].endpoint[2],self.propsB[i].endpoint[4]));
    	if 3 >= math.abs(self.propsB[i].endpoint[2]-self.propsB[i].endpoint[1]) then
          ...
          -- p_vision:add_debug_message(string.format("L%d post fail ",i));
          -- p_vision:add_debug_message(string.format("(%d %d)->(%d %d) post check fail\n",
        	-- self.propsB[i].endpoint[1],self.propsB[i].endpoint[3],
        	-- self.propsB[i].endpoint[2],self.propsB[i].endpoint[4]));  -- (1,3) (2, 4)  = (x1, y1) (x2, y2)
        	valid = false
        end
      end
    end
    ...
    if valid then
      ...
      self.length[linecount]=self.propsB[i].length;
  ...
  end --ending for loop with nLines
  nLines = linecount;
  ...
  end -- ending for loop with nLines (second one)
  ...
  end -- ending for loop with nLines (third one)
...
local update_shm = function(self, parent_vision)
  ...
  if (self.detect == 1) then
    ...
    vcm['set_line'..cidx..'_angle'](self.angle[max_index]);
		vcm['set_line'..cidx..'_angleTable'](self.angle);
