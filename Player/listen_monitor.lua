...
function push_cbScaleB(obj, nc)
  local name = parse_name(obj.name);
  local cbScaleB = cutil.test_array();

  if (FIRST_CB_SCALEB == true) then
    print("initiate cbScaleB flag");
    local cbScaleB_flag = vector.zeros(name.parts);
    FIRST_CB_SCALEB = false;
  end
  cbScaleB_flag[name.partnum] = 1;
  cbScaleB_all[name.partnum] = obj.data;

	--print('before if')
	--print('name.partnum: '..name.partnum)
	--print('name.parts: '..name.parts)
  if (check_flag(cbScaleB_flag) and name.partnum==name.parts ) then
		--print('after if')
    local cbScaleB_str = "";
    for i = 1 , name.parts do --fixed
      cbScaleB_str = cbScaleB_str .. cbScaleB_all[i];
    end

    cutil.string2userdata(cbScaleB, cbScaleB_str);
    vcm['set_image'..nc..'_cbScaleB'](cbScaleB);
		--print('setting_image_cbScaleBBBB')
  end
  -- cutil.string2userdata(cb, obj.data);
  -- vcm['set_image'..nc..'_cb'](cb);
end

function push_cb(obj, nc)
  local name = parse_name(obj.name);
  local cb = cutil.test_array();

  if (FIRST_CB == true) then
    print("initiate cb flag");
    local cb_flag = vector.zeros(name.parts);
    FIRST_CB = false;
  end
  cb_flag[name.partnum] = 1;
  cb_all[name.partnum] = obj.data;
	
	--print('before if')
	--print('name.partnum: '..name.partnum)
	--print('name.parts: '..name.parts)
  if (check_flag(cb_flag) and name.partnum==name.parts ) then
		--print('after if')
    local cb_str = "";
    for i = 1 , name.parts do --fixed
      cb_str = cb_str .. cb_all[i];
    end

    cutil.string2userdata(cb, cb_str);
    vcm['set_image'..nc..'_cb'](cb);
		--print('setting_image_cb AAAAAA')
  end
  -- cutil.string2userdata(cb, obj.data);
  -- vcm['set_image'..nc..'_cb'](cb);
end
  ...
  if( msg ) then
    ...
    if( obj.arr ) then
      ...
      for nc = 1, 2 do
        ...
        elseif ( string.find(obj.arr.name,'cb'..tostring(nc)) ) then
          push_cb(obj.arr, nc);
				elseif ( string.find(obj.arr.name,'cbScaleB'..tostring(nc)) ) then
					push_cbScaleB(obj.arr, nc)
       	end
      end
    else
    	push_data(obj);
