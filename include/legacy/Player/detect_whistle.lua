...
--[[
this function returns the value of auto_correlation, 
which is the multiplied value of sound value at time t and time t_shift
]] 
...
--[[
this function uses autocorrelate and find the period where the return value from previous function
is maximized. This is considered to be the period of the detected sound data;
...
while true do
  ...
  if (gcm.get_game_state() == 2 and gcm.get_game_whistle() == 0) then 
    -- initialize 
    -- go into actual functions if the game is in Set state and whistle flag has been turned on
