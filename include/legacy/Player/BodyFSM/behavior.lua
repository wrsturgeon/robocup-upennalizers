module(..., package.seeall);

require('Body')
require('walk')
require('vector')
require('Config')
require('wcm')
require('gcm')
require('position')

----------------------------------
-------- Explain variables -------
----------------------------------
-- kickDir:{1 = front kick; 2 = side kick}
-- kickType:{1 = stationary kick; 2 = walk kick}
-- fsm.playMode:{1 = demo; 2 = simple behav with orbit; 3 = advanced behav}
--tKickOffWear = Config.team.tKickOffWear or 30.0;
thSideKick1 = Config.fsm.thSideKick1 or 45*math.pi/180;
thSideKick2 = Config.fsm.thSideKick2 or 135*math.pi/180;
thDistSideKick = Config.fsm.thDistSideKick or 3.0; --threshold distance for sideKick(if ball distance is < than this, kick sideways)
tKickOffWear = Config.team.tKickOffWear or 30.0;
doWalkKick = walk.canWalkKick and Config.fsm.enable_walkkick -- should enable walk kick?
postYellow = Config.world.postYellow;
postCyan = Config.world.postCyan;

--In demo mode show different kicks
function cycle_behavior()
  demo_behavior = demo_behavior%4 + 1;
  if demo_behavior == 1 then
    --Speak.talk("Front kick test");
    kickDir = 1;
    kickType = 1;
  elseif demo_behavior == 2 then
    --Speak.talk("Side kick test");
    kickDir = 2;
    kickType = 1;
  elseif demo_behavior == 3 then
    --Speak.talk("Front walkkick test");
    kickDir = 1;
    kickType = 2;
  elseif demo_behavior == 4 then
    --Speak.talk("Side walkkick test");
    kickDir = 2;
    kickType = 2;
  end
end
--end of cycle_behavior()

--Initial kick for demo
if Config.fsm.playMode == 1 then
  demo_behavior = 0;
  cycle_behavior();
end

function update()
  -----------------------------------------------------------
  -- Kickoff handling
  ------------------------------
  t = Body.get_time(); -- sensor time
  --kickDir: 1 front, 2 to the left, 3 to the right
  --kickType: 1 stationary kick, 2 walkkick

  -------------------------------
  --Kickoff handling
  ------------------------------
  tKickOffWear = Config.team.tKickOffWear or 30.0;


  t = Body.get_time(); --get current time
  kick_off = wcm.get_kick_kickOff();
  tKickOff = wcm.get_kick_tKickOff();

  --If too long time has passed since game starts
  --Don't care about kickoff kick
  if (t - tKickOff) > tKickOffWear and kick_off == 1 then
    print("kickoff weared off")
    wcm.set_kick_kickOff(0);
    kick_off = 0;
  end


  -- skip kick selection in demo mode
  if Config.fsm.playMode == 1 then --demo mode
    if kickDir > 1 then
      kickDir = 5 - kickDir; --Switch sidekick direction for demo mode
    end
    kickAngle = 0;
  else -- normal mode
    if kick_off > 0 then
      kickAngle = 10*math.pi/180; -- kickoff angle
      kickDir = 1;
      kickType = 2;
      wcm.set_kick_dir(kickDir);
      wcm.set_kick_type(kickType);
      wcm.set_kick_angle(kickAngle);
      return;
    end

    position.posCalc(); -- calculate postion
    aGoal = wcm.get_goal_attack_angle2();
    pose = wcm.get_pose();
    angleRot = util.mod_angle(aGoal - pose.a);

    --Check if front walkkick is available now
    kickType = 2;
    ball = wcm.get_ball();

    --rBall = distance of ball from robot
    rBall = math.sqrt(ball.x^2 + ball.y^2);

    if rBall > thDistSideKick or math.abs(angleRot) < thSideKick1 or math.abs(angleRot)>thSideKick2 then
      --print("STRAIGHT",angleRot*180/math.pi)
      kickDir = 1;
      kickAngle = 0;
    elseif angleRot > 0 then -- kick to the left
      kickDir = 2;
      kickAngle = 70*math.pi/180;
    else -- kick to the right
      kickDir=3;
      kickAngle = -70*math.pi/180;
    end
    if rBall > thDistSideKick or
       math.abs(angleRot) < thSideKick1 or
       math.abs(angleRot) > thSideKick2 then
      kickDir = 1;
      kickAngle = 0;
    elseif angleRot > 0 then --should kick to the left
      kickDir = 2;
      kickAngle = 70*math.pi/180;
    else
      kickDir = 3;
      kickAngle = -70*math.pi/180;
    end
  end


  if not doWalkKick then
    kickType = 1;
  end


  --If sidekick is disabled, set kickDir = 1 {disable sidekick}
  if not Config.fsm.enable_sidekick and kickDir ~= 1 then
    kickDir = 1;
    kickAngle = 0;
  end

  --If robot is confused
  --Always kick to the side
  --Always use the walkkick and walk sidekick
  is_confused = wcm.get_robot_is_confused() ~= 0;
  if is_confused then
    kickType = 2;
    if math.abs(angleRot) < 40*math.pi/180 or math.abs(angleRot) > 140*math.pi/180 then
      -- We are facing one of the goal.... do the sidekick
      print("CONFUSED: SIDE KICK", angleRot*180/math.pi)
      --TODO: kick towards OUTSIDE of the field
      kickDir = 2; --Sidekick to the left
      kickAngle = 90*math.pi/180;
    else  --We are facing sideways, do straight kick
      print("CONFUSED: FRONT KICK", angleRot*180/math.pi)
      kickDir = 1;
      kickAngle = 0*math.pi/180; --Straight kick
    end
  end

  wcm.set_kick_dir(kickDir);
  wcm.set_kick_type(kickType);
  wcm.set_kick_angle(kickAngle);
end
--end of update() function



function get_attack_bearing_pose(pose0)
  if gcm.get_team_color() == 1 then
    -- red attacks cyan goal
    postAttack = postCyan;
  else
    -- blue attack yellow goal
    postAttack = postYellow;
  end

  -- make sure not to shoot back towards defensive goal:
  local xPose = math.min(math.max(pose0.x, -0.99*PoseFilter.xLineBoundary),
                          0.99*PoseFilter.xLineBoundary);
  local yPose = pose0.y;
  local aPost = {}
  aPost[1] = math.atan2(postAttack[1][2]-yPose, postAttack[1][1]-xPose);
  aPost[2] = math.atan2(postAttack[2][2]-yPose, postAttack[2][1]-xPose);
  local daPost = math.abs(util.mod_angle(aPost[1]-aPost[2]));

  attackHeading = aPost[2] + .5*daPost;
  attackBearing = PoseFilter.mod_angle(attackHeading - pose0.a);

  return attackBearing, daPost;
end
