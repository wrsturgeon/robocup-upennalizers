...
local state = utilMsgMixed.get_default_state()
...
function recv_msgs_new_comm()
  ...
  if Config.dev.comm == 'TeamComm' then
     ...
     local count;
     while msg do
           t = utilMsgMixed.convert_state_std_to_penn(msg);
...
function build_state()
  ...
  utilMsgMixed.pack_vision_info(state)
  randind = 1 
  ...
  utilMsgMixed.pack_labelB_TeamMsg(state, randind)
...
function update()
    ...
    if t-tLastSent > 1/send_fps then
        ...
        if Config.dev.comm == 'WebotsComm' then
            ...
            msg = utilMsgMixed.convert_state_penn_to_std(state)
            Comm.send(msg)      
        end
    ...
    for id = 1,5 do       
        ...
        if states[id] and states[id].tReceive and (t - states[id].tReceive < msgTimeout) then
            ...
            if id == playerID then
                ball = wcm.get_ball();
                ballProb[id] = ball.p;
            elseif util.InTable(Config.robot_ids_penn,id) then
                ballProb[id] = cur_state.ball.p;
                print("Penn bot p:",ballProb[id])
            else
                print('UT bot t: ',tBall)
                if tBall < 0.3 then
