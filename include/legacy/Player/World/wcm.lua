...
shared.robot.temperature = vector.zeros(1); --leg temperature velue
shared.robot.temperatureSide = vector.zeros(1); --0 for left, 1 for right
shared.robot.cd = vector.zeros(1);

-- Bottom camera covered by shoulder
--0 = No
--1 = Partially covered
--2 = Majority/Completely covered
shared.robot.shoulder_covered = vector.zeros(1);
...
shared.team.isFrontDef = vector.zeros(1);
...
shared.team.total_confidence = vector.ones(5);
shared.team.my_confidence = vector.ones(5);
shared.team.pos_confidence = vector.ones(5); -- sent via UDP
...
--NOT IN USE BUT CAN BE USED FOR ATTACKINNG FREEKICK STRATEGY
--0 = No Freekick
--1 = Freekick for us
--2 = Freekick for them
shared.kick.freeKick = vector.zeros(1); --just Added


--Added for obstacle avoidance for webots
shared.obstacle = {};
shared.obstacle.num = vector.zeros(1);
shared.obstacle.x = vector.zeros(13);
shared.obstacle.y = vector.zeros(13);
shared.obstacle.dist = vector.zeros(13);
shared.obstacle.role = vector.zeros(13);
shared.obstacle.radius = vector.zeros(13);
...
--7 = Kick-in for us
--8 = Kick-in for them
shared.obstacle.foulType = vector.zeros(1);
shared.obstacle.foulTime = vector.zeros(1);
shared.obstacle.foulConf = vector.zeros(1); --do we know where the foul is. 0=no, 1=yes

-- Kickoff obstacle (does the other team have kickoff)
--
shared.obstacle.kickOffTime = vector.zeros(1);
...
function set_foulType(x)
  shared.obstacle.foulType[1] = x;
end

function get_foulType()
  return shared.obstacle.foulType[1];
end

function set_foulTime(x)
  shared.obstacle.foulTime[1] = x;
end

function get_foulTime()
  return shared.obstacle.foulTime[1];
end

--NOT IN USE BUT CAN BE USED FOR ATTACKINNG FREEKICK STRATEGY

function set_freeKick(x)
  shared.kick.freeKick[1] = x;
end

function get_freeKick()
  return shared.kick.freeKick[1];
end
