...
print_localization_update = true
...
rLCornerFilterTop = Config.world.rLCornerFilterTop or 0.01;
aLCornerFilterTop = Config.world.aLCornerFilterTop or 0.03;
rTCornerFilterTop = Config.world.rTCornerFilterTop or 0.01;
aTCornerFilterTop = Config.world.aTCornerFilterTop or 0.03;
rLCornerFilterBtm = Config.world.rLCornerFilterBtm or 0.01;
aLCornerFilterBtm = Config.world.aLCornerFilterBtm or 0.03;
rTCornerFilterBtm = Config.world.rTCornerFilterBtm or 0.01;
aTCornerFilterBtm = Config.world.aTCornerFilterBtm or 0.03;
rSpotFilter = Config.world.rSpotFilter or 0.03;
aSpotFilter = Config.world.aSpotFilter or 0.06;
aCircleFilterTop = Config.world.aCircleFilterTop or 0.02;
rCircleFilterTop = Config.world.rCircleFilterTop or 0.01;
aCircleFilterBtm = Config.world.aCircleFilterBtm or 0.02;
rCircleFilterBtm = Config.world.rCircleFilterBtm or 0.01;--wLineFilter = Config.world.wLineFilter 
...
function initialize(p0, dp)
  ...
  if print_localization_update then
...
--Adjust particles based on observation of a L corner in bottom camera
function cornerLBtm(v,a)
  if (rLCornerFilterBtm>0 and aLCornerFilterBtm>0) then
    if print_localization_update then
      log.info('L Corner Update - bottom')
    end
	  if(Config.game.playerID == 1) then
	    oriented_object_observation(Lgoalie_corner,v,a,rLCornerFilterBtm,aLCornerFilterBtm);
	  else
	    oriented_object_observation(Lcorner,v,a,rLCornerFilterBtm,aLCornerFilterBtm);
	  end
   end
end


--Adjust particles based on observation of a T corner in bottom camera
function cornerTBtm(v,a)
  if (rTCornerFilterBtm>0 and aTCornerFilterBtm>0) then
    if print_localization_update then
      log.info('T Corner Update - bottom')
    end
	  if(Config.game.playerID == 1) then
	    oriented_object_observation(Tgoalie_corner,v,a,rTCornerFilterBtm,aTCornerFilterBtm);
	  else
	    oriented_object_observation(Tcorner,v,a,rTCornerFilterBtm,aTCornerFilterBtm);
	  end
   end
end

--Adjust particles based on observation of a L corner in top camera
function cornerLTop(v,a)
  if (rLCornerFilterTop>0 and aLCornerFilterTop>0) then
    if print_localization_update then
      log.info('L Corner Update - top')
    end
	  if(Config.game.playerID == 1) then
	    oriented_object_observation(Lgoalie_corner,v,a,rLCornerFilterTop,aLCornerFilterTop);
	  else
	    oriented_object_observation(Lcorner,v,a,rLCornerFilterTop,aLCornerFilterTop);
	  end
   end
end


--Adjust particles based on observation of a T corner in top camera
function cornerTTop(v,a)
  if (rTCornerFilterTop>0 and aTCornerFilterTop>0) then
    if print_localization_update then
      log.info('T Corner Update - top')
    end
	  if(Config.game.playerID == 1) then
	    oriented_object_observation(Tgoalie_corner,v,a,rTCornerFilterTop,aTCornerFilterTop);
	  else
	    oriented_object_observation(Tcorner,v,a,rTCornerFilterTop,aTCornerFilterTop);
	  end
   end
end
...
function topCircleUpdate(v,a)
   if (rCircleFilterTop>0 and aCircleFilterTop>0) then
     if print_localization_update then
      log.info('Top circle Update')
     end
      oriented_circle(v,a,rCircleFilterTop,aCircleFilterTop);
   end
end

function btmCircleUpdate(v,a)
   if (rCircleFilterBtm>0 and aCircleFilterBtm>0) then
     if print_localization_update then
      log.info('Btm circle Update')
     end
      oriented_circle(v,a,rCircleFilterBtm,aCircleFilterBtm);
   end
end
...
--Adjust particles based on observation of the top spot
function topSpotUpdate(v,a)   
  if (rSpotFilter>0 and aSpotFilter>0) then
     if print_localization_update then
       log.info('top SPOT update')
     end
     landmark_observation(spotWhite, v, rSpotFilter, aSpotFilter, 0);
  end
end

--Adjust particles based on observation of the bottom spot
function bottomSpotUpdate(v)
  if (rSpotFilter>0 and aSpotFilter>0) then
     if print_localization_update then
       log.info('bottom SPOT update')
     end
     landmark_observation(spotWhite, v, rSpotFilter, aSpotFilter, 0);
  end
end
