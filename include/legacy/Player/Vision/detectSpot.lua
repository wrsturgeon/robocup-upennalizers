...
local update = function(self, color, p_vision)
  ...
  self.detect = 0;
  if cidx == 1 then
    min_area = math.floor((math.sqrt(Config.vision.spot.min_area)*2)^2)
    max_area = math.floor((math.sqrt(Config.vision.spot.max_area)*2)^2)
    ground_boundingbox = {} 
    for i=1,4 do
      ground_boundingbox[i] = Config.vision.spot.ground_boundingbox[i]*2
    end
  end
  ...
  for i = 1,#spotPropsB do
    ...
    spotStats_green = ImageProc.color_stats(p_vision.labelA.data, 
      p_vision.labelA.m, p_vision.labelA.n, Config.color.field, bboxA);
    ...
    if valid then
      ...
      if black_rate_A > max_black_rate_A then
       ...
       fill_rate_undefined = 1 - (spotStats.area + spotStats_green.area) / spotArea_A;
       if fill_rate_undefined > 0.1 then
         --p_vision:add_debug_message(string.format("black + undefined rate: %.2f\n", fill_rate_undefined))
         valid = false;
       end
    end
    ...
    if valid then
      spotCentroid = HeadTransform.coordinatesA(spotStats.centroid, 1);
      spotCentroid = HeadTransform.projectGround(spotCentroid,0);
      if spotCentroid[1] > 1.5 then
        --p_vision:add_debug_message(string.format("Spot too far \n"));
        valid = false;
      end
    end

    if valid then
      local ballCentroid = vcm.get_ball_v() -- ball_centroid is in terms of image row-col, ball_v is in terms of distance from robot
      if ballCentroid then
        local diff = spotCentroid - ballCentroid
        local dist = math.sqrt(diff[1]*diff[1] + diff[2]*diff[2])
        if dist < 0.15 then
          --p_vision:add_debug_message(string.format("Spot too close to ball \n"));
          valid = false;
        end
      end
    end
...
local update_shm = function(self, parent_vision)
  ...
  vcm['set_spot'..cidx..'_detect'](self.detect)
  if self.detect == 1 then
    vcm['set_spot'..cidx..'_v'](self.v)
    vcm['set_spot'..cidx..'_bboxB'](self.bboxB);
    vcm['set_spot'..cidx..'_color'](Config.color.white)
  end
end
