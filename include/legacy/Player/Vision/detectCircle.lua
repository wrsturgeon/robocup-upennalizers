...
local update = function(self,p_vision,line)
	...
	if cidx == 1 then
		if line.detect == 0 or line.nLines<5 then
	...
	if (centerLineExists == 1) then
  	for i=1,line.nLines do
    	if i ~= idx then
				--p_vision:add_debug_message(string.format(" L%d length: %.2f", i, length[i]));
        --local angle_test = getCircleAngle(line, i, idx);
        --p_vision:add_debug_message(string.format(" L%d angle: %.2f", i, angle_test));

        if (cidx == 1) and (length[i] > 1.7) and (getCircleAngle(line, i, idx) < angle_threshold) then
          --p_vision:add_debug_message(string.format(" L%d length too high. No circle detected.", i));
          return;
        end
...
local update_shm = function(self, parent_vision)
	...
	vcm['set_circle'..cidx..'_detect'](self.detect);
	if (self.detect==1) then
		vcm['set_circle'..cidx..'_x'](self.x_center);
		vcm['set_circle'..cidx..'_y'](self.y_center);
		vcm['set_circle'..cidx..'_var'](self.var);
		vcm['set_circle'..cidx..'_angle'](self.circle_angle);
	end
end
