...
if( ~isempty(r_mon) )
    if cidx==1 % TOP CAMERA
      scale_mul = 1;
        if((r_mon.ball.detect==1) && (r_mon.ball.detecttop == 1) && (r_mon.ball.detectbtm == 0))
            hold on;
...
%         if ((r_mon.line.detect == 1) && (r_mon.line.detectbtm == 0))
        if (r_mon.linetop.detect == 1)
            hold on;
%             plot_line(r_mon.line,scale*4)
            plot_line(r_mon.linetop,scale*4)
            hold off;
        end
        if ((r_mon.corner.detect == 1) && (r_mon.corner.detecttop == 1))
% 	    if ((r_mon.corner.detect == 1) && (r_mon.corner.detectbtm == 0))
	        hold on;
	        plot_corner(r_mon.corner, scale*2)
	        hold off;
        end

        if(( r_mon.spot.detect == 1) && (r_mon.spot.detecttop == 1) && r_mon.spot.detectbtm == 0)
            hold on;
            plot_spot(r_mon.spot,scale)
            hold off;
        end

        if (r_mon.balltop.newBbox==1)
            plot_ball_regions_proposed(r_mon.balltop.bboxLeftTopX, r_mon.balltop.bboxLeftTopY, ...
                r_mon.balltop.bboxRightBottomX, r_mon.balltop.bboxRightBottomY, cidx);
        end
      ...
      scale_mul = 4;
        ...
        if(( r_mon.spot.detect == 1) && (r_mon.spot.detectbtm == 1))
            hold on;
            plot_spot(r_mon.spot,scale)
            hold off;
        end
%         if ((r_mon.line.detect == 1) && (r_mon.line.detectbtm == 1))
        if (r_mon.linebtm.detect == 1)
            hold on;
%             plot_line(r_mon.line,scale*4)
            plot_line(r_mon.linebtm,scale*4)
            hold off;
        end
        if (r_mon.corner.detect == 1) && (r_mon.corner.detectbtm == 1)
            hold on;
            plot_corner(r_mon.corner,scale*2)
            hold off;
        end
        ...
        if (r_mon.ballbtm.newBbox==1)
            plot_ball_regions_proposed(r_mon.ballbtm.bboxLeftTopX, r_mon.ballbtm.bboxLeftTopY, ...
                r_mon.ballbtm.bboxRightBottomX, r_mon.ballbtm.bboxRightBottomY, cidx);
        end
...
%Subfunctions
    ...
    function plot_ball_regions_proposed(bboxLeftTopX, bboxLeftTopY, bboxRightBottomX, bboxRightBottomY, cidx)
        n_bbox=0;
        if all([length(bboxLeftTopX), length(bboxLeftTopY), length(bboxRightBottomX)] == length(bboxRightBottomY))
            n_bbox = length(bboxLeftTopX);
        end
        if n_bbox > 0
            hold on
            for i=1:n_bbox
                rectangle('Position', [bboxLeftTopX(i), bboxLeftTopY(i), ...
                    abs(bboxRightBottomX(i)-bboxLeftTopX(i)), abs(bboxRightBottomY(i)-bboxLeftTopY(i))])
            end
            hold off
        end
    end
    ...
    function plot_ball( ballStats, scale )
        ...
        if ballStats.fromRP == 1
            plot( centroid(1), centroid(2),'c+','MarkerSize',12,'LineWidth',3) % changed to red for easier seeing
        else
    ...
    function plot_spot(spotStats,scale)
        ...
        plot([bl br],[(bb+bt)/2 (bb+bt)/2],'m','LineWidth',5);
        plot([(bl+br)/2 (bl+br)/2],[bt bb],'m','LineWidth',5);
