...
-- Check all blobs until hit a ball that no longer passes area check 
  for i=1,#ballPropsB do
    ...
    local pink = 7;
    self.pink = ImageProc.color_stats(p_vision.labelA.data, p_vision.labelA.m, p_vision.labelA.n, pink, bboxA);
    ...
    if self.propsA.area > self.th_max_color2 then
    ...
    elseif (self.propsA.area / vcm.bboxArea(self.propsA.boundingBox)) < self.th_min_fill_rate then
      ...
      check_passed = false;
    elseif aspect_ratio>4 or aspect_ratio<0.25 then
...
--      self.propsA.area,fill_rate));
      ...
      local area_diff_error = exp_area * 0.3
...
--      local ballv = {v_inf[1],v_inf[2],0};
    ...
    if check_passed then
        print('Robot Pink area is '..self.pink.area);
