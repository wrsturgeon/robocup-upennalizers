...
% Level 5: wireless (level 2 + robot name)
  ...
  if robot_struct.fall
    ...
    if drawlevel==1
      ...
      plot_ball(robot_struct, r_mon ,scale);
...
%    set(b_name,'FontSize',32/scale);
  ...
  function plot_ball(robot, r_mon, scale)
    if (~isempty(robot.ball) && r_mon.ball.detect == 1)
...
%      marker2 = strcat(marker,'--');
  ...
  function plot_spot(spot,scale)
    if (~isempty(spot) && (spot.detect==1))
      v = spot.v;
      xb = x0 + v(1)*ca - v(2)*sa;
      yb = y0 + v(1)*sa + v(2)*ca;
      %hb = plot(xb, yb, [idColors(robot_struct.id) 'o']);
