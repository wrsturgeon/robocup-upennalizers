...
Remember above how variables are required to take up space in the computer? That's only true for things you can _assign to_. If you write `x = f()`, `x` is an _lvalue_ (since it's on the __**l**__eft), and `f()` is an `rvalue` (since it's on the __**r**__ight). Lvalues stay lvalues, though: in `x = f(); y = x;`, the second use of `x` (on the right-hand side of an equation) is still an lvalue. So we arrive at a better definition of lvalues: _they_ take up assignable space in your computer, and rvalues are everything else.
